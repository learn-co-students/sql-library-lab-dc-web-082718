...
def select_name_and_series_subgenres_of_authors
  ...
  FROM series
  JOIN authors ON series.author_id = authors.id JOIN subgenres ON series.subgenre_id = subgenres.id"
end
...
def select_series_title_with_most_human_characters
  ...
  # "SELECT series.title, COUNT(characters.id)
  # FROM series
  # JOIN books ON books.series_id
  # JOIN character_books ON character_books.book_id = books.id
  # JOIN characters ON characters.series_id = character_books.character_id
  # GROUP BY series.title HAVING characters.species = 'human'
  # ORDER BY COUNT(characters.id) DESC LIMIT 1"
end
